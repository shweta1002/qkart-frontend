...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response = await axios.post(`${config.endpoint}/user/addresses`,{ address: newAddress },{headers:{Authorization: `Bearer ${token}`}});
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const response = await axios.post(`${config.endpoint}/cart/checkout`,{addressId:addresses.selected},{headers:{Authorization:`Bearer ${token}`}});
      enqueueSnackbar("Order placed successfully",{variant:"success"});
      const walletBalance = parseInt(localStorage.getItem("balance") - getTotalCartValue(items));
  ...
  const FetchAddresses = () => {
    ...
    return addresses.all.map((address) => (
      <Box key={address._id} className={addresses.selected === address._id ? "address-item selected" : "address-item not-selected"} onClick={()=>{setAddresses({...addresses, selected:address._id})}}>
        <Typography>{address.address}</Typography>
        <Button startIcon={<Delete />} onClick={async () => { await deleteAddress(token, address._id)}} >Delete</Button>
      </Box>
    ));
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const userAddresses = getAddresses(token);

...
export const generateCartItemsFrom = (cartData, productsData) => {
  if(!cartData) return;
  const cartProduct = cartData.map((item)=> ({
    ...item,
...
export const getTotalCartValue = (items = []) => {
  ...
  const totalCost = items.map((item) => item.cost*item.qty).reduce((totalCost, n) => totalCost+n);
 ...
 */
 export const getTotalItems = (items = []) => {
    ...
    const countProducts = items.reduce((count, obj)=> {
          ...
          {item.qty > 0 ? (
            ...
            >
                ...
                <Box
                    ...
                    alignItems="center"
                >
                <ItemQuantity value={item.qty} isReadOnly={isReadOnly}
                // Add required props by checking implementation
                handleAdd={ async () => {
                  await handleQuantity(token,items,products,item._id,item.qty+1,{preventDuplicate:(token) ? false : true})
                } }
                handleDelete={ async () => {
                  await handleQuantity(token,items,products,item._id,item.qty-1,{preventDuplicate:(token) ? false : true})
                }}
